...
func TestGetUserUsageStatsAggregatesPerUser(t *testing.T) {
	...
	assert.Equal(t, UserUsageStat{UserId: 1, Username: "alice", RequestCount: 2, PromptTokens: 30, CompletionTokens: 10, TotalTokens: 40, Quota: 150}, stats[0])
...
func TestGetUserUsageStatsMergesRenamedUserIntoOneRow(t *testing.T) {
	truncateTables(t)
	seedUsageLog(t, Log{UserId: 1, Username: "alice", Type: LogTypeConsume, CreatedAt: 1000, Quota: 10})
	seedUsageLog(t, Log{UserId: 1, Username: "alice-renamed", Type: LogTypeConsume, CreatedAt: 1100, Quota: 20})

	stats, err := GetUserUsageStats(1000, 2000)
	require.NoError(t, err)
	require.Len(t, stats, 1)
	assert.Equal(t, 1, stats[0].UserId)
	assert.Equal(t, "alice-renamed", stats[0].Username)
