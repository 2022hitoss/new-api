...
func TestRequestPolicyDatabaseMatrix(t *testing.T) {
	for _, dialect := range []string{"sqlite", "mysql", "postgres"} {
		t.Run(dialect, func(t *testing.T) {
			...
			assert.Equal(t, "false", CurrentRequestPolicy().Options["service_tier_policy.reject_enabled"], "the service tier toggle is exported with its default")
			assert.Equal(t, "fast\npriority", CurrentRequestPolicy().Options["service_tier_policy.blocked_tiers"], "the blocked tier list is exported with its default")
			require.NoError(t, UpdateRequestPolicyOptions(map[string]string{"service_tier_policy.reject_enabled": "true", "service_tier_policy.blocked_tiers": "priority\nflex"}))
			assert.True(t, operation_setting.GetServiceTierPolicySetting().RejectEnabled, "the runtime struct follows the same write")
			assert.Equal(t, "priority\nflex", operation_setting.GetServiceTierPolicySetting().BlockedTiers)
			assert.Error(t, UpdateRequestPolicyOptions(map[string]string{"service_tier_policy.reject_enabled": "maybe"}), "an invalid boolean is rejected before any write")
			assert.True(t, operation_setting.GetServiceTierPolicySetting().RejectEnabled)
			assert.Error(t, UpdateRequestPolicyOptions(map[string]string{"service_tier_policy.other": "true"}), "only allowlisted service tier keys are accepted")
			loadOptionsFromDatabase()
			assert.True(t, operation_setting.GetServiceTierPolicySetting().RejectEnabled, "the persisted toggle survives a reload")
			assert.Equal(t, "priority\nflex", CurrentRequestPolicy().Options["service_tier_policy.blocked_tiers"])
